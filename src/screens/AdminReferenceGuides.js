...
const AdminReferenceGuides = ({ navigation }) => {
  ...
  const loadReferenceGuides = async () => {
    try {
      const guides = await firebaseService.getReferenceRanges();
      if (!guides || guides.length === 0) {
        // Initialize with default ranges if none exist
        console.log("No reference guides found, initializing with defaults");
        setReferenceGuides(guides);
      } else {
        setReferenceGuides(guides);
      }
    } catch (error) {
  ...
  const saveReferenceGuide = async (edit) => {
    try {
      // Save updated guides to Firebase
      if (edit) {
        await firebaseService.updateReferenceRange(
          currentGuide.id,
          currentGuide
        );
        Alert.alert("Success", "Reference guide updated");
        setReferenceGuides(
          referenceGuides.map((guide) =>
            guide.id === currentGuide.id ? currentGuide : guide
          )
        );
      } else {
        await firebaseService.addReferenceRange(currentGuide);
        Alert.alert("Success", "Reference guide added");
      }
      setVisible(false);
      resetCurrentGuide();
    } catch (error) {
  ...
  const deleteGuide = async (id) => {
    try {
      await firebaseService.deleteReferenceRange(id);
      Alert.alert("Success", "Reference guide deleted");
      setReferenceGuides(referenceGuides.filter((guide) => guide.id !== id));
    } catch (error) {
  ...
  const resetToDefaults = async () => {
    try {
      await Promise.all(
        DEFAULT_REFERENCE_RANGES.map((range) =>
          firebaseService.addReferenceRange(range)
        )
      );
              ...
              <DataTable>
                ...
                <DataTable.Row>
                  ...
                  <DataTable.Cell numeric>
                    {formatValue(guide.IgG.min, IMMUNOGLOBULIN_UNITS.IgG)}
                  </DataTable.Cell>
                  <DataTable.Cell numeric>
                    {formatValue(guide.IgG.max, IMMUNOGLOBULIN_UNITS.IgG)}
                  </DataTable.Cell>
                  <DataTable.Cell numeric>
                    {IMMUNOGLOBULIN_UNITS.IgG}
                  </DataTable.Cell>
                </DataTable.Row>
                ...
                <DataTable.Row>
                  ...
                  <DataTable.Cell numeric>
                    {formatValue(guide.IgA.min, IMMUNOGLOBULIN_UNITS.IgA)}
                  </DataTable.Cell>
                  <DataTable.Cell numeric>
                    {formatValue(guide.IgA.max, IMMUNOGLOBULIN_UNITS.IgA)}
                  </DataTable.Cell>
                  <DataTable.Cell numeric>
                    {IMMUNOGLOBULIN_UNITS.IgA}
                  </DataTable.Cell>
                </DataTable.Row>
                ...
                <DataTable.Row>
                  ...
                  <DataTable.Cell numeric>
                    {formatValue(guide.IgM.min, IMMUNOGLOBULIN_UNITS.IgM)}
                  </DataTable.Cell>
                  <DataTable.Cell numeric>
                    {formatValue(guide.IgM.max, IMMUNOGLOBULIN_UNITS.IgM)}
                  </DataTable.Cell>
                  <DataTable.Cell numeric>
                    {IMMUNOGLOBULIN_UNITS.IgM}
                  </DataTable.Cell>
                </DataTable.Row>
                ...
                <DataTable.Row>
                  ...
                  <DataTable.Cell numeric>
                    {formatValue(guide.IgE.max, IMMUNOGLOBULIN_UNITS.IgE)}
                  </DataTable.Cell>
                  <DataTable.Cell numeric>
                    {IMMUNOGLOBULIN_UNITS.IgE}
                  </DataTable.Cell>
                </DataTable.Row>
              </DataTable>
        ...
        >
          <ScrollView>
            <Text style={styles.modalTitle}>
              {currentGuide.id ? "Edit" : "Add"} Reference Guide
            </Text>
            ...
            <View style={styles.modalButtons}>
              <Button
                mode="contained"
                onPress={() =>
                  saveReferenceGuide(currentGuide.id ? true : false)
                }
