...
class FirebaseService {
  ...
  async getAllTestResults() {
    try {
      // إنشاء استعلام لجلب جميع التحاليل بترتيب حسب تاريخ الإنشاء
      const q = query(
  ...
  async getPatient(patientId) {
    try {
      ...
      return patientDoc.exists()
        ? { id: patientDoc.id, ...patientDoc.data() }
        : null;
    } catch (error) {
  ...
  async updateReferenceRange(id, updatedData) {
    try {
      const rangeDocRef = doc(db, "referenceRanges", id);
      await updateDoc(rangeDocRef, updatedData);
      console.log(`Reference range with ID: ${id} updated successfully.`);
      return true;
    } catch (error) {
      console.error(`Error updating reference range with ID: ${id}:`, error);
      throw error;
    }
  }
  async deleteReferenceRange(id) {
    try {
      const rangeDocRef = doc(db, "referenceRanges", id);
      await deleteDoc(rangeDocRef);
      console.log(`Reference range with ID: ${id} deleted successfully.`);
      return true;
    } catch (error) {
      console.error(`Error deleting reference range with ID: ${id}:`, error);
      throw error;
    }
  }
  ...
  async queryTestResults(filters = {}) {
    try {
      let q = collection(db, "testResults"); // قاعدة الاستعلام الأساسية
      ...
      const queryRef = query(q, ...conditions);
      // تنفيذ الاستعلام
